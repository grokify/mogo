...
// CondenseSpace trims space from lines and removes empty lines. `unique` dedupes lines and `sort`
// preforms a sort on the results.
func CondenseSpace(elems []string, dedupeResults, sortResults bool) []string {
	results := TrimSpace(elems, true)
	if dedupeResults {
...
// TrimSpace removes leading and trailing spaces per string. If condense
...
func CondenseAndQuoteSpace(s []string, quoteLeft, quoteRight string) []string {
	return CondenseAndQuote(s, " ", " ", quoteLeft, quoteRight)
}
...
// Compare returns 3 slices given 2 slices which represent intersection
...
func Compare(sliceA, sliceB []string) ([]string, []string, []string) {
	...
	return CondenseSpace(anob, true, true),
		CondenseSpace(both, true, true),
		CondenseSpace(bnoa, true, true)
}
...
// ToMap returns the slide where the slice elements are the keys of the map,
...
// ToDoc converts a slice to a map, trimming the values if desired. The `cfg` keys are
...
func IntersectionCondenseSpace(slice1, slice2 []string) []string {
	return Intersection(
		CondenseSpace(slice1, true, false),
		CondenseSpace(slice2, true, false))
}

// IsEmpty checks to see if a slice is empty. If `skipEmptyStrings`
...
// OrderExplicit reoders the values of a slice using a requested input order
...
/*
// DedupeOrdered is a function with similar functionality to SliceOrderExplicit().
func DedupeOrdered(s, ordered []string, sortAsc bool, inclUnordered bool) []string {
...
// SplitLengthStats returns a `map[int]int` indicating how many
...
// dBySplitLength returns lines by split length. This is useful for analyzing
