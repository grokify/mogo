...
func ZonesSystem(zoneDirs []string) []string {
	// Adapted from https://stackoverflow.com/a/40130882/1908967
	zones := []string{}
	for _, zoneDir := range zoneDirs {
		zones = readZoneFile(zoneDir, "", zones)
	}
	return listutil.StripSliceElements(zones, []string{"+VERSION"})
}
...
func readZoneFile(zoneDir, path string, zones []string) []string {
	...
	for _, f := range files {
		...
		if f.IsDir() {
			zones = readZoneFile(zoneDir, path+"/"+f.Name(), zones)
		} else {
...
func ZoneOffsetSeconds(dt time.Time, ianaTimeZoneName string) (int, error) {
	zone, err := time.LoadLocation(ianaTimeZoneName)
	if err != nil {
		return -1, err
	}
	t2 := dt.In(zone)
